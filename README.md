...
긴 YouTube 영상 전체를 정리하려면 데스크톱 영상 설명 영역에서 **더보기(More) → 스크립트 표시(Show transcript)**를 선택합니다. 자막 패널은 영상 오른쪽에 열립니다. 직접 열지 않은 경우에는 에이전트가 관찰된 동일 컨트롤을 최대 2회 조작으로 찾도록 요청할 수 있습니다. 열린 자막이 길면 에이전트는 최신·기존 YouTube 자막 구간을 모두 인식해 타임스탬프별로 나누고 장별·전체 요약으로 합칩니다. 전체 영상 분석을 위해 영상을 끝까지 재생하거나 종료를 기다리지는 않습니다. 진행 중인 구간 수가 Side Panel 상태에 표시되며 중지 버튼으로 취소할 수 있습니다.
...
- YouTube는 현재 보이는 프레임, 플레이어 상태, 현재 표시 자막을 분석합니다. 데스크톱에서는 `더보기(More) → 스크립트 표시(Show transcript)` 뒤 영상 오른쪽에 열린 최신 `transcript-segment-view-model`과 기존 자막 구간을 최대 8,000자 청크와 계층형 요약으로 처리합니다. 전체 영상 요청에서는 재생 제어보다 이 요약을 우선하며 영상 종료를 기다리지 않습니다. 전체 스크립트가 없으면 관찰된 동일 컨트롤을 최대 2회 조작으로 탐색합니다. 좁은 화면이나 다른 영상 사이트에서는 오른쪽 위치를 단정하지 않고 실제 관찰된 배치를 따릅니다. 사이트가 열린 자막 구간을 구조화된 DOM으로 제공하지 않으면 현재 관찰 범위만 안내하며, 구간별 자동 프레임 샘플링은 후속 범위입니다.
