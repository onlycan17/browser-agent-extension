...
Chrome Side Panel에서 현재 탭을 분석하고, 안전 정책과 사용자 승인을 거쳐 클릭·텍스트 입력·폼 제어·중첩 스크롤·YouTube 제어를 수행하는 Manifest V3 확장 프로그램입니다.

## 주요 기능

- 현재 뷰포트의 보이는 텍스트와 상호작용 요소 구조화(편집 중인 입력·초안 제외)
- 요청별 동의를 받은 PNG 화면 캡처와 provider-safe 페이지 정보를 한 메시지로 묶는 멀티모달 모델 분석
- 관찰 시점의 일회성 element ID, 가림 검사, 실행 직전 DOM 가드를 이용한 클릭·텍스트 입력·키·스크롤
- 옵션 라벨 기반 select 선택, 체크박스·라디오 상태 변경, 모달·목록 내부 스크롤
- 클릭·Enter·폼 상태 변경 뒤 DOM이 잠잠해질 때까지 bounded 대기 후 최신 화면 재관찰
- Content Script의 구조화된 오류 코드를 보존해 stale·가림·안전 차단별 재시도 여부 판단
- 최대 단계, 즉시 중단, 반복 실패 차단을 포함한 LLM tool-call 에이전트
- 전송·구매·삭제·로그인·외부 이동 전 사용자 승인
- 비밀번호·결제 카드·인증 코드·파일 입력 차단
- YouTube 상태·현재 자막 분석, 열린 긴 자막의 구간별·계층형 요약, bounded 스크립트 탐색, 재생·정지·탐색·속도·볼륨 제어
...
OpenAI, Anthropic, OpenRouter, Groq, Together AI, DeepSeek, Mistral, xAI는 API key가 필수이며, 누락 시 네트워크 요청 전에 안내합니다. Local과 Custom은 서버 정책에 따라 key 없이 사용할 수 있습니다.
...
6. 요청과 필요한 첨부파일을 입력하고 **보내기**를 선택한다. 에이전트는 직접 답변, 안전한 페이지 동작, 필요할 때의 화면 캡처를 하나의 흐름에서 판단한다.
7. 승인 카드가 나타나면 대상과 이유를 확인한 뒤 한 번 승인하거나 거부한다.

긴 YouTube 영상 전체를 정리하려면 영상 설명 영역에서 관찰되는 **스크립트 표시(Show transcript)**를 선택합니다. 현재 배치에 따라 **더보기(More)** 뒤에 있을 수도 있습니다. 직접 열지 않은 경우에는 에이전트가 관찰된 동일 컨트롤을 최대 2회 조작으로 찾도록 요청할 수 있습니다. 열린 자막이 길면 에이전트는 최신·기존 YouTube 자막 구간을 모두 인식해 타임스탬프별로 나누고 장별·전체 요약으로 합칩니다. 전체 영상 분석을 위해 영상을 끝까지 재생하거나 종료를 기다리지는 않습니다. 진행 중인 구간 수가 Side Panel 상태에 표시되며 중지 버튼으로 취소할 수 있습니다.
...
- 화면 캡처는 현재 보이는 탭 영역만 지원합니다. **멀티모달 화면 분석 허용**은 이미지 입력을 지원하는 모델에서만 켜야 하며 요청이 끝나면 효력이 사라집니다.
- Chrome 내부 페이지, Chrome Web Store, 다른 확장 UI는 조작할 수 없습니다.
- shadow DOM과 cross-origin iframe 내부는 관찰하지 않습니다.
- YouTube는 현재 보이는 프레임, 플레이어 상태, 실시간·길이 확인 상태, 현재 표시 자막을 분석합니다. 관찰된 `스크립트 표시(Show transcript)` 또는 `더보기(More) → 스크립트 표시`로 열린 최신 `transcript-segment-view-model`과 기존 자막 구간을 최대 8,000자 청크와 계층형 요약으로 처리합니다. 메뉴나 패널 위치를 고정하지 않고 실제 관찰을 따르며, 안정 구간 키와 종료 재확인으로 동적으로 늘어나는 자막을 이어 읽습니다. 사이트가 열린 자막 구간을 구조화된 DOM으로 제공하지 않으면 현재 관찰 범위만 안내하며, 구간별 자동 프레임 샘플링은 후속 범위입니다.
...
- `docs/TEST_PLAN.md`
- `docs/TODOLIST.md`
- `docs/ADAPTIVE_RELIABILITY_AND_VISION_DESIGN.md`
- `HARNESS_PLAN.md`
