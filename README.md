...
- 관찰 시점의 일회성 element ID와 실행 직전 DOM 가드를 이용한 클릭·텍스트 입력·키·스크롤
- 최대 단계, 즉시 중단, 반복 실패 차단을 포함한 LLM tool-call 에이전트
- 전송·구매·삭제·로그인·외부 이동 전 사용자 승인
- 비밀번호·결제 카드·인증 코드·파일 입력 차단
- YouTube 상태·현재 자막 분석, 관찰된 전체 스크립트 우선 사용과 bounded 탐색, 재생·정지·탐색·속도·볼륨 제어
...
- YouTube는 현재 보이는 프레임, 플레이어 상태, 현재 표시 자막을 분석합니다. 영상 전체 분석에서 전체 스크립트가 이미 관찰되면 이를 우선 사용하고, 없으면 관찰된 `More > Show transcript` 계열 컨트롤을 최대 2회 조작으로 탐색합니다. 찾지 못하면 현재 관찰 범위를 안내하며, 구간별 자동 프레임 샘플링은 후속 범위입니다.
