...
- 사용자가 요청 단위로 허용하면 개인정보가 제거된 DOM 관찰 결과와 PNG 데이터 URL을 같은 멀티모달 메시지로 이미지 입력 지원 모델에 전달하고, 제출 즉시 동의를 해제한다.
...
- 성공했지만 페이지 안정화가 확인되지 않은 동작은 같은 실행에서 다시 수행하지 않는다.

### 3.3 YouTube 분석 및 제어

- YouTube 영상의 제목, URL, 현재 시간, 길이, 길이 확인 여부, 실시간 여부, 재생 상태, 속도, 볼륨을 읽는다.
- 재생, 일시 정지, 길이가 유한하게 확인된 영상의 특정 시간 이동, 재생 속도 및 볼륨 변경을 지원한다.
- 현재 프레임은 탭 캡처로 분석한다.
- 페이지에서 자막 텍스트를 안전하게 얻을 수 있는 경우 분석 컨텍스트에 포함한다.
- 영상 전체 분석 요청에서는 관찰 결과에 이미 전체 스크립트가 있으면 컨트롤을 조작하지 않고 이를 우선 사용한다.
- 열린 전체 스크립트가 길면 타임스탬프 경계의 최대 8,000자 구간으로 나누고, 구간 요약을 6개 단위로 계층적으로 합쳐 전체 요약을 만든다. 종료 안정화가 확인되지 않으면 확인한 범위만 부분 요약으로 표시한다.
...
- 전체 스크립트가 보이지 않으면 에이전트는 관찰된 영상 설명의 `스크립트 표시(Show transcript)` 또는 `더보기(More) → 스크립트 표시` 경로를 최대 2회 조작으로 탐색하고 페이지를 재관찰한다.
- YouTube, 다른 영상 사이트, 좁은 화면 모두 고정된 메뉴·패널 위치를 단정하지 않고 관찰된 Transcript/Script 컨트롤과 실제 배치를 따른다.
...
- 같은 전환뿐 아니라 서로 다른 동작이 같은 페이지 상태를 반복하는 순환도 감지해 1회 bounded re-planning 기회를 제공하고, 계속 정체되면 안전 한도에서 중단한다.
...
- 동일 전환 또는 입력 외 동작이 바뀌어도 페이지 상태가 3회 연속 그대로이면 정체로 판단해 중단한다.
...
- 사용자가 요청 단위 멀티모달 화면 분석을 허용하면 agent가 현재 보이는 viewport의 새 screenshot을 모델 tool로 요청할 수 있다.
- screenshot tool은 pinned tab 검증, Chrome capture rate 제한, run당 6회 제한, 기존 취소·시간·단계 안전 한도를 적용하고 provider-safe 페이지 관찰과 이미지를 한 메시지로 묶는다.
